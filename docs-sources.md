...
| Fremdaufruf URL-Builder (EN notice page) | `V.connect/Fremdaufruf/url-builder.md` | this repo | canonical | n/a — stub pointing at the German version |
| Fremdaufruf Architecture | `V.connect/Fremdaufruf/solution-outline.md` | `mcp-health/V.connect` → `fremdaufruf/solution-outline.md` | yes | manual copy — transform on sync: the source links `specification.md` and `../2fa/findings.md` as siblings, which they are in that repo but not here. Neither is published; replace both with prose naming the file and the repo, as the mirrored copy does |
| Fremdaufruf URL-Builder (DE) | `V.connect/Fremdaufruf/url-builder.de.md` | `Vertama-GmbH/elim` (historical name; the V.ap repo) → `docs/external/v.connect/fremdaufruf/url-builder.md` | yes | manual copy — note: the source now writes cross-references as absolute `docs.vertama.com` URLs (V.ap-side readers cannot resolve paths in this repo). On sync, re-base them to repo-relative `.md` paths so MkDocs keeps validating them at build time; this replaces the earlier re-basing of broken relative paths after the V.connect uplift (2026-08) |
| Fremdaufruf Betriebshandbuch | — | `mcp-health/V.connect` → `fremdaufruf/manual/index.html` | **no** — ships with binary; admin listener serves it at `/admin/manual/` | n/a |
